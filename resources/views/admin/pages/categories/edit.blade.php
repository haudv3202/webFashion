                        ...
                        <div class="card">
                            <div class="card-body flex flex-col p-6">
                                ...
                                <div class="card-text h-full ">
                                    <form class="space-y-4" action="{{ route('categories.update') }}" method="post" enctype="multipart/form-data">
                                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-7">
                                            @csrf
                                            <input type="hidden" name="id" value="{{ $data->id }}">
                                            <div class="input-area relative">
                                                <label for="largeInput" class="form-label">Tên Danh mục</label>
                                                <input type="text" name="titleCategories" class="form-control" value="{{ old('titleBanner',$data->name) }}" placeholder="Full Title">
                  ...
                  <span class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap">
                    <span id="placeholder" class="text-slate-400">Choose a file or drop it here...</span>
                              ...
                              <span class="file-name flex-none cursor-pointer border-l px-4 border-slate-200 dark:border-slate-700 h-full inline-flex items-center bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-400 text-sm rounded-tr rounded-br font-normal">Browse</span>
                                                    ...
                                                    <div id="file-preview">
                                                        <img src="{{ asset($data->image_categories) }}" alt="">
                                                    </div>
                                                </div>
