                        ...
                        <div class="card">
                            <div class="card-body flex flex-col p-6">
                                ...
                                <div class="card-text h-full ">
                                    <form class="space-y-4" action="{{ route('admin.slide.PostSlide') }}" method="post" enctype="multipart/form-data">
                                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-7">
                                            @csrf
                                            <div class="input-area relative">
                                                <label for="largeInput" class="form-label">Tiêu đề banner</label>
                                                <input type="text" name="titleBanner" class="form-control" value="{{ old('titleBanner') }}" placeholder="Full Title">
                                                @error('titleBanner')
                                            ...
                                            <div class="input-area relative">
                                                ...
                                                <input type="text" name="titleSale" class="form-control" value="{{ old('titleSale') }}" placeholder="Title Sale">
                                                @error('titleSale')
                                                <div class="py-[18px] mt-4 px-6 font-normal font-Inter text-sm rounded-md bg-danger-500 bg-opacity-[14%] text-danger-500">
                                                    <div class="flex items-start space-x-3 rtl:space-x-reverse">
                                                        <div class="flex-1">
                              ...
                              <span class="file-name flex-none cursor-pointer border-l px-4 border-slate-200 dark:border-slate-700 h-full inline-flex items-center bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-400 text-sm rounded-tr rounded-br font-normal">Browse</span>
                                            ...
                                            <div class="input-area">
                                                ...
                                                <select id="select" name="status" class="form-control">
                                                    ...
                                                    <option value="1" {{ old('status') == 1 ? 'selected' : '' }} class="dark:bg-slate-700">Ẩn</option>
                                                    <option value="2" {{ old('status') == 2 ? 'selected' : '' }} class="dark:bg-slate-700">Hiện</option>
                                                </select>
                                                @error('status')
                                                <div class="py-[18px] mt-4 px-6 font-normal font-Inter text-sm rounded-md bg-danger-500 bg-opacity-[14%] text-danger-500">
                                                    <div class="flex items-start space-x-3 rtl:space-x-reverse">
